      ...
      <section class="my-5">
        ...
        <?php   
          if(isset($_GET['product_id'])){
            ...
            $pdetail_run   = sqlsrv_query($conn,$pdetail_query,$params,$options);

            if(sqlsrv_num_rows($pdetail_run) > 0){
              $pdetail_row = sqlsrv_fetch_array($pdetail_run);
                $pid = $pdetail_row['pid'];
        ...
        ?>
          ...
          <div class="col-md-7">
            ...
            <?php
			 ...
			 $cat_run   = sqlsrv_query($conn,$pdetail_query,$params,$options);
			 
              if(sqlsrv_num_rows($cat_run) > 0){
                $cat_row = sqlsrv_fetch_array($cat_run); 
            ...
            if(isset($_SESSION['email'])){
                 $custid = $_SESSION['id'];
                  if(isset($_POST['submit'])){
					...
					$run_cart    =  sqlsrv_query($conn,$sel_cart,$params,$options);

                    if(sqlsrv_num_rows($run_cart) == 0){
                      $cart_query = "INSERT INTO cart(cust_id, product_id,quantity) VALUES ($custid,$p_id,$qty)";    
					  
                     //if(mysqli_query($con,$cart_query)){
					 if (sqlsrv_query($conn,$cart_query,$params,$options)){
...
//                     }
                    ...
                    if(sqlsrv_num_rows($run_cart) > 0){
                        while($row = sqlsrv_fetch_array($run_cart)){
        ...
        <div class="row mt-5 mb-4">
          ...
          <!-- Grid column -->
           <?php 
               ...
               $p_query = "SELECT TOP 4 * FROM furniture_product WHERE category LIKE '%$category%' order by title DESC";
                //$p_run   = mysqli_query($con,$p_query);
				$p_run    =  sqlsrv_query($conn,$p_query,$params,$options);
                  
                  if(sqlsrv_num_rows($p_run) > 0 ){
                      while($p_row = sqlsrv_fetch_array($p_run)){
                       $pid      = $p_row['pid'];
                       $ptitle  = $p_row['title'];
                       $pcat    = $p_row['category'];
                       $p_price = $p_row['price'];
