  ...
  if(isset($_SESSION['id'])){
    ...
    if(isset($_GET['cart_id'])){
      ...
      //$run_cart    = mysqli_query($con,$sel_cart);
		if (($rs=sqlsrv_query($conn,$sel_cart,$params,$options)) == false)
		{
			echo  print_r(sqlsrv_errors(), true);
		}
            ...
            <div class="col-md-3 col-12">
                ...
                <?php
                   ...
                   $cat_query = "SELECT * FROM categories ORDER BY id ASC";
					...
					if (($rs=sqlsrv_query($conn,$cat_query,$params,$options)) == false)
					{
						echo  print_r(sqlsrv_errors(), true);
					}
				  ...
				  if(sqlsrv_num_rows($rs)>0){
					while($cat_row=sqlsrv_fetch_array($rs)) {
                ...
                <?php   
                  ...
                  if(isset($_GET['cat_id'])){
					...
					if (($rs=sqlsrv_query($conn,$cat_query,$params,$options)) == false)
					{
					  ...
					  $p_query = "SELECT * FROM furniture_product WHERE category=$catid ORDER BY pid DESC OFFSET $product_start ROWS FETCH NEXT $required_pro ROWS ONLY";  
                    ...
                    $p_query = "SELECT * FROM furniture_product WHERE status='publish' ORDER BY pid DESC OFFSET $product_start ROWS FETCH NEXT $required_pro ROWS ONLY";  
					
                  }
                 
                  //$p_run   = mysqli_query($con,$p_query);
					if (($p_run=sqlsrv_query($conn,$p_query,$params,$options)) == false)
					{
						echo  print_r(sqlsrv_errors(), true);
					}
