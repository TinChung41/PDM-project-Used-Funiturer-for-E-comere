 ...
 <?php  
   if(isset($_SESSION['id'])){
      ...
      if(isset($_POST['checkout'])){
		...
		$run  = sqlsrv_query($conn,$cartt,$params,$options);
        ...
        if(mysqli_num_rows($run) > 0){
		  ...
		  $pr_run  = sqlsrv_query($conn,$pr_query,$params,$options);
		  
          if(sqlsrv_num_rows($pr_run) > 0){
            while($pr_row = sqlsrv_fetch_array($pr_run)){
              ...
              $single_pro_total_price,'$invoice',$db_pro_qty,'$date','pending')";
                  ...
                  //if(mysqli_query($con,$checkout_query)){
					if(sqlsrv_query($conn,$checkout_query,$params,$options)){
                        $del_query = "DELETE FROM cart where cust_id = $customer_id";
                        //if(mysqli_query($con,$del_query)){
						if(sqlsrv_query($conn,$del_query,$params,$options)){
            ...
            ?>
               ...
               <div class="col-md-6 p-3">
                 ...
                 <?php
				  ...
				  $run  = sqlsrv_query($conn,$cart,$params,$options);
				  
                   if(sqlsrv_num_rows($run) > 0){
                      while($cart_row = sqlsrv_fetch_array($run)){
